...
// Normalize absolute image paths for pandoc resource resolution
let md = fs.readFileSync(output, 'utf8')
md = md.replace(/!\[\]\([^)]*?media\/media\//g, '![](media/media/')
md = md.replace(/!\[[^\]]*\]\([^)]*?media\/media\//g, (m) => m.replace(/^!\[[^\]]*\]\([^)]*?media\/media\//, '![](media/media/'))
