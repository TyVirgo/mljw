/**
 * 基于「培养方案变更说明书」模板章节结构，生成学籍管理 V2.0 vs V2.4 菜单对比 docx
 * 保留模板章节标题格式（一级菜单/二级子菜单 + 字段/排版/内容三维度），仅填充对比正文
 */
import fs from 'fs'
import {
  Document,
...
async function main() {
  ...
  addMenu(2, '复学', noChange(), diff3('Tab 壳层内', '四个标签页组成的申请页面结构内', '容器描述变更，布局不变'), noChange())

  addMenu(
    2,
    '退学',
    diff3('原因来自 WDR001 类别 reasons', '原因来自「退学异动类别配置的原因列表」', '原因来源表述变更'),
