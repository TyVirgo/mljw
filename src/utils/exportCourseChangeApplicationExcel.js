...
export const courseChangeExportColumns = [
...
export const courseChangeExportFields = courseChangeExportColumns
...
export function exportCourseChangeApplicationsToExcel(
  items,
  filename = 'course-change-application.xlsx',
  selectedFieldKeys = courseChangeExportFields.map((f) => f.key),
) {
  const columns = courseChangeExportColumns.filter((col) => selectedFieldKeys.includes(col.key) || col.key === 'no')
  ...
  XLSX.utils.book_append_sheet(workbook, worksheet, 'Course Change Application')
