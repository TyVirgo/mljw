...
import {
  downloadStudentProfileImportTemplate,
  parseStudentProfileImportFile,
  exportStudentImportErrorReport,
  buildStudentImportErrorReportFilename,
} from '../../utils/importStudentProfileExcel.js'
...
async function handleImport() {
  ...
  try {
    const parsed = await parseStudentProfileImportFile(selectedFile.value, props.existingStudents)
    ...
    if (parsed.successRows.length && !parsed.errorRows.length) {
      ...
      result.value = {
        type: 'success',
        message: t('studentProfile.importSuccessCount', { count: parsed.successCount }),
      }
      return
    }
    ...
    if (parsed.successRows.length && parsed.errorRows.length) {
      ...
      result.value = {
        type: 'warning',
        message: t('studentProfile.importPartialSuccess', {
    ...
    result.value = {
      type: 'error',
      message: t('studentProfile.importFailedCount', { count: parsed.errorCount }),
    }
  } catch {
    result.value = { type: 'error', message: tr('Failed to parse import file.') }
  } finally {
    importing.value = false
  }
}
...
function downloadErrorReport() {
  ...
  exportStudentImportErrorReport(lastErrorRows.value, buildStudentImportErrorReportFilename())
}
</script>
      ...
      <div class="modal-panel" role="dialog" aria-modal="true" @click.stop>
        ...
        <div class="modal-body">
          ...
          <div v-if="result" class="result-box" :class="result.type">
            ...
            <button v-if="lastErrorRows.length" type="button" class="link-btn" @click="downloadErrorReport">
