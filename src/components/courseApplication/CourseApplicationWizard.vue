...
function loadApplication(application) {
  const state = applicationToWizardState(application)
...
function validateStep1() {
  const excludeId = isEditMode.value ? props.initialApplication?.id : null
  const validationErrors = validateApplicationGeneralForm(form.value, props.allApplications, excludeId)
...
const allCLOSelected = computed(() => clos.value.length > 0 && clos.value.every((item) => cloSelectedIds.value.includes(item.id)))
...
<template>
  <div class="course-application-wizard" :class="{ 'is-detail': isReadonly }">
    ...
    <CourseDetailStepper :model-value="currentStep" :steps="courseCreateSteps" @update:model-value="goToStep" />

    <div class="wizard-body">
      <section v-show="currentStep === 1" class="step-section">
        <template v-if="isReadonly">
          <CourseGeneralInfoDetail :data="form" />
        </template>
      ...
      <CourseSLTStepPanel v-show="currentStep === 3" v-model:slt="slt" :clos="clos" :readonly="isReadonly" />
    </div>

    <CoursePrerequisiteModal
      :visible="prerequisiteModalVisible"
      :courses="allApplications"
...
<style scoped>
.course-application-wizard.is-detail {
