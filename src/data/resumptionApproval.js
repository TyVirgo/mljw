import {
  canApproveResumption,
  formatResumptionDateTime,
} from './resumptions.js'
...
export const commonApprovalComments = [
  ...
  'Please attach updated medical clearance or payment receipt.',
  'Outstanding dues not cleared.',
  'Programme intake is not available for the requested semester.',
  'Rejected due to policy restrictions on resumption.',
]
...
export function applyApprovalDecisionToItem(item, action, comment) {
  if (!canApproveResumption(item)) return item
