...
- [x] 8.1 `studentRecordsMenu.js`：删除 `sr-study-plan-group` 整组；移除 `buildStudentRecordsBreadcrumbKeys` study-plan 分支
- [x] 8.2 `App.vue`：`sidebarExpandedGroups` 去掉 `sr-study-plan-group`
- [x] 8.3 `scripts/prd/prd-module-intros.mjs`：一级菜单介绍改为两组导航
- [x] 8.4 冒烟：侧边栏仅「学籍管理」「学籍异动」两组；无「学生个人培养方案」
